...
void setup(void) {
  ...
  pinMode(4, OUTPUT); //led on digital 4
...
void loop(void) {
  ...
  if(((unsigned long)(millis()-starttime) >= sampletime_ms) || measure){
    if ( !ds.search(addr)) {
    ...
    digitalWrite(4, HIGH); //led on when writing
  ...
  //  Serial.println();
    ...
    digitalWrite(4, LOW); //led off when not writing
